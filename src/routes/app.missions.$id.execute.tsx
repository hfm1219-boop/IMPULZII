...
function ExecutePage() {
  ...
  useEffect(() => {
    ...
    const ex = ExecutionService.activeForMission(user.id, mission.id)
      ?? ExecutionService.accept(user.id, mission.id);
    ExecutionService.start(ex.id);
    setExecId(ex.id);
  ...
  const totalSteps = requiredFields.length + (mission?.requiresPhoto ? 1 : 0) + (mission?.requiresGeo ? 1 : 0);
  const doneSteps =
    filledRequired +
  ...
  // autosave
  useEffect(() => {
    if (execId) ExecutionService.saveDraft(execId, answers, evidences, loc ?? undefined);
  }, [answers, evidences, loc, execId]);
    ...
    <div className="space-y-4">
      ...
      <div className="sticky top-0 z-10 bg-background pt-2 pb-3">
        <div className="flex items-center justify-between text-xs mb-1">
          ...
          <span className="text-muted-foreground">{doneSteps}/{totalSteps}</span>
        </div>
        <Progress value={progress} />
      </div>

      <Card className="p-4 space-y-4">
        {mission.fields.sort((a, b) => a.order - b.order).map((f) => (
          <DynamicField
            key={f.id}
            field={f}
            value={answers[f.id]}
            ...
            onPhoto={(dataUrl) => {
              ...
              setEvidences((prev) => [...prev.filter((e) => e.type !== "photo" || e.id !== ev.id), ev]);
            }}
          />
        ))}
      </Card>
      ...
      {mission.requiresGeo && (
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold flex items-center gap-2"><MapPin className="h-4 w-4" /> Ubicación</p>
              {loc ? (
            ...
            <Button type="button" variant={loc ? "outline" : "default"} onClick={requestLocation} disabled={locStatus === "loading"}>
