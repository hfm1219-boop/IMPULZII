    ...
    <div className="space-y-4">
      ...
      <Card className="p-5 gradient-brand text-primary-foreground border-0">
        ...
        <div className="mt-4 grid grid-cols-3 gap-3 text-xs">
          <div><div className="opacity-70">Pendiente</div><div className="font-semibold">{data.balance.pending}</div></div>
          <div><div className="opacity-70">Acumulado</div><div className="font-semibold">{data.balance.total}</div></div>
          <div><div className="opacity-70">Redimido</div><div className="font-semibold">{data.balance.redeemed}</div></div>
        </div>
      </Card>
      ...
      <section className="space-y-2">
        ...
        {data.tx.length === 0 ? (
          <EmptyState title="Sin movimientos" description="Completa una misión para ganar tus primeros puntos." />
        ) : (
          data.tx.map((t) => (
